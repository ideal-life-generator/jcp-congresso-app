...
atea.controller 'RateController', [ '$scope', '$location', 'baseURL', '$routeParams', 'connectionTest', '$filter', '$compile', 'message', 'getDataTest', '$http',
($scope, $location, baseURL, $routeParams, connectionTest, $filter, $compile, message, getDataTest, $http) ->
		...
		if $scope.surveyForm.$valid
			...
			if result.length
				getDataTest.save { resource: 'surveyAnswer' }, { data: records: result }, (result) ->
					message.warning "Your form has been saved. Thanks."
					getDataTest.put { resource: 'participant' }, { data: id: $scope.participient.id, extraParam: addTokens: 'passageSurvey' }, (result) ->
						data = result.data
						message.success "You have received #{data.message.receivedTokens} tokens!", ->
...
atea.controller 'RatesController', [ '$scope', '$location', 'baseURL', '$routeParams', 'connectionTest', 'message',
($scope, $location, baseURL, $routeParams, connectionTest, message) ->
